...
def _rewrite_twimg_images(body: str) -> str:
    """Rewrite Twitter/X `pbs.twimg.com` image refs to a display-friendly
    size variant and add a width constraint to the markdown alt text.

    Idempotent — re-running on already-rewritten content is a no-op
    (`name=medium` already, `|600` already present in alt).
    """
    def _sub(m):
        bang, alt, mid, url, close = m.groups()
        # Step A: downsize the URL parameter
        new_url = _TWIMG_NAME_PARAM_RE.sub(r'\1medium', url)
        # Step B: ensure the alt text carries a width constraint. Match
        # the canonical Obsidian form `|<digits>` at the end of alt text;
        # if absent, append `|600`. Preserve any caller-set width.
        if re.search(r'\|\d+$', alt):
            new_alt = alt
        else:
            new_alt = (alt.rstrip() + '|600') if alt.strip() else 'Image|600'
        return f'{bang}{new_alt}{mid}{new_url}{close}'
    return _TWIMG_IMG_RE.sub(_sub, body)
