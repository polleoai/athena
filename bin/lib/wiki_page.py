    ...
    if url:
        if source_icon:
            if '/' in source_icon:
                ...
                page += f'{source_icon} '
        page += f'[Source]({url}) \u00B7 '
    local_copy = local_copy_path or raw_path
...
def generate_url_tracker(vault_path):
    ...
    # Recently Added — wikilinks to the local wiki page, not the source URL.
    # Captured entries have a corresponding wiki page keyed by URL (NOT
    # title — TSV titles are raw HTML page titles that often contain
    # '&amp;', ' | site name', etc., which collide with Obsidian wikilink
    # syntax and don't match the canonical wiki filename ('Web: <title>').
    # Build a URL→wiki-filename index by scanning wiki frontmatter, then
    # use that to emit a wikilink that actually resolves.
    if all_ordered:
        ...
        def _wikilink_for(entry):
            ...
            if stem:
                return stem
            # Fallback: sanitize title — html-decode + replace pipes/dashes
            # so the link is at least readable when it doesn't resolve.
            t = _html.unescape((entry.get('title') or '').strip())
            return t.replace('|', '—').replace('  ', ' ').strip() or 'Untitled'

        recent = all_ordered[-20:][::-1]
        page += f'## Recently Added ({len(all_ordered)} total, showing last 20)\n\n'
        for e in recent:
            page += f'- [[{_wikilink_for(e)}]]\n'
        page += '\n'
...
def create_wiki_page(vault_path, raw_path, url=None, llm_result=None,
    ...
    wiki_path = os.path.join(wiki_dir, clean_title + '.md')
    ...
    if not overwrite and os.path.exists(wiki_path):
        if _existing_page_matches_url(wiki_path, url):
            return {'status': 'exists', 'page_name': clean_title, 'summary': None,
