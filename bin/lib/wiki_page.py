...
def generate_page_index(vault_path):
    ...
    for wpath in glob.glob(os.path.join(vault_path, 'wiki', '**', '*.md'), recursive=True):
        ...
        try:
            ...
            continue
        # Group by max(date_added, last_updated) so re-captured pages
        # bubble up to today's section instead of staying frozen under
        # their first-capture date. Falls back to date / created if
        # neither preferred field exists. None at all → 'Undated'.
        date_added_m = re.search(r'^\s*date_added:\s*"?(\d{4}-\d{2}-\d{2})', head, re.MULTILINE)
        last_updated_m = re.search(r'^\s*last_updated:\s*"?(\d{4}-\d{2}-\d{2})', head, re.MULTILINE)
        candidates = []
        if date_added_m: candidates.append(date_added_m.group(1))
        if last_updated_m: candidates.append(last_updated_m.group(1))
        if not candidates:
            for key in ('date', 'created'):
                ...
                if m:
                    ...
                    break
        # YYYY-MM-DD lexical sort = chronological; max = most recent.
        date_str = max(candidates) if candidates else ''
...
if __name__ == '__main__':
    ...
    if args.command == 'journal':
        ...
        result = create_wiki_page(
            ...
            # 1.0.15: support overwrite=True from stdin so the JS-side
            # `kb refresh <url>` chat command can regenerate a stale
            # wiki page (bypasses dedup; snapshots the prior page to
            # .kb-trash/ before writing — reversible via kb undo).
