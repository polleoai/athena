...
const KB_COMMANDS = [
  ...
  { cmd: "kb regen <url>",             desc: "Regenerate the wiki digest for a page" },
  { cmd: "kb regen --all-pending",     desc: "Fill in every \"Pending synthesis\" page" },
  { cmd: "kb rename <page> --to \"N\"",desc: "Rename page + update links" },
  { cmd: "kb remove <page>",           desc: "Soft-delete page + raw to trash" },
...
const STATUS_MAP = {
  ...
  rules: "Loading rules...",
  regen: "Synthesizing wiki digest...",
};
...
function detectMechanicalCommand(text) {
  ...
  const patterns = [
    ...
    // 1.1: kb regen — fill in or rewrite the wiki digest by re-running
    // synthesis against the raw source. Different from kb refresh, which
    // re-runs the wiki PAGE BUILDER (frontmatter, source line, structure)
    // but leaves the digest body alone. Regen targets only the digest.
    { pattern: /^kb\s+regen\s+(https?:\/\/\S+)$/i, command: "regen", argsMatch: 1 },
    { pattern: /^kb\s+regen\s+--all-pending$/i, command: "regen", args: ["--all-pending"] },
    ...
    // `kb remove <page>` always removes the raw source alongside the
    // wiki page — there's no reason to keep an orphan raw whose wiki
    // companion is gone. (The previous `--with-raw` opt-in was a no-op
    // anyway; the JS handler always took both.)
