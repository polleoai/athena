...
// Gryphon is consumed as a git submodule at vendor/gryphon. All Athena-side
// code that extends the chat UI goes through Gryphon's documented
// extension points (options bag on GryphonChatView) — never via
// subclassing or reaching into Gryphon internals.
...
class AthenaPlugin extends Plugin {
  async onload() {
    ...
    // Register the view — composition, not inheritance. Athena configures
    // a GryphonChatView instance through the options bag. Extension points:
    ...
    this.registerView(VIEW_TYPE, (leaf) => {
      ...
      const claudeCodeArgs = usesClaudeCodeCLI
        ...
        : [];
      return new GryphonChatView(leaf, this, {
        viewType: VIEW_TYPE,
        displayText: "Athena",
        icon: ICON,
      ...
      if (mechanical.command === "add") {
        ...
        if (result.status === "duplicate") {
          ...
          if (result.pageName) {
            ...
            lines.push(`**Captured:** ${url}`);
            lines.push("Page added to knowledge base.");
          }
          view.finalizeStreamingMessage(
            lines.join("\n"),
            result.pageName ? `Page created: ${result.pageName}` : "Page added"
          );
        }
        view.addCostInfo(0, null);
        if (view.inputEl) view.inputEl.focus();
  ...
  async ingestContent(opts) {
    ...
    if (cleanUrl && !rawContent) {
      ...
      const browserText = await this.browserCapture(cleanUrl);
      if (browserText) {
        ...
        if (captureOutput.includes("Saved:") || captureOutput.includes("already exists")) {
          ...
          return { status: "failed", pageName: null, summary: "Capture failed. Check the URL and try again." };
        }
      }
  ...
  _runWikiPageBuilder(input) {
    const vaultPath = this.app.vault.adapter.basePath;
    const scriptPath = path.join(vaultPath, "bin", "lib", "wiki_page.py");
    return new Promise((resolve) => {
      const proc = spawn("python3", [scriptPath, "--stdin"], {
        cwd: vaultPath,
        env: { ...process.env, PATH: buildEnhancedPath() },
        stdio: ["pipe", "pipe", "pipe"],
      });
      ...
      proc.on("close", (code) => {
        clearTimeout(timer);
        if (stderr) console.log("[athena] wiki_page.py stderr:", stderr.substring(0, 200));
        try {
...
- summary: 2-3 sentences, not one-liners. Focus on the key insight, not generic description`;
    ...
    return new Promise((resolve) => {
      ...
      const proc = spawn(claudePath, ["-p", prompt, "--model", model, "--max-turns", "1"], {
    ...
    try {
      ...
      try {
        const out = execFileSync("python3", [
          path.join(vaultBase, "bin/lib/process_clip.py"),
          vaultBase,
          filePath,
