...
const _BROWSER_EXTRACT_JS = `
  (function() {
    ...
    // X.com tweets get the focused text-extraction; everything else
    // falls back to the article/main/body progression.
    var text = '';
    var tweets = document.querySelectorAll('[data-testid="tweetText"]');
    if (tweets.length > 0) {
      text = Array.from(tweets).map(function(el) { return el.innerText; }).join('\\n\\n');
    } else {
      var article = document.querySelector('article, main, [role="main"]');
      text = article ? article.innerText : (document.body.innerText || '');
    }

    // Image extraction scoped to the main content area when present.
    // Filters: only HTTPS srcs (no data:/blob:/relative), and skip
    // images smaller than 50x50 to avoid tracking pixels, favicons,
    // and spacer GIFs. Dedupe by src.
...
class AthenaPlugin extends Plugin {
  ...
  async ingestContent(opts) {
    ...
    if (cleanUrl && !rawContent) {
      ...
      if (browserResult) {
        ...
        // Build images-as-HTML markdown for the body tail. Same width=600
        // + alt="..." shape as the twimg rewrite so the visual style is
        // consistent across hosts. HTML-escape attribute values
        // defensively against any embedded quotes.
        const _htmlAttr = (s) => String(s || "").replace(/"/g, "&quot;");
        const imagesMd = browserImages.length > 0
  ...
  /** kb remove <page> [--with-raw]. Soft-deletes to .kb-trash/ and
   *  updates TSV status. Default is wiki page + raw together (always
   *  --with-raw conceptually — the raw is the source-of-truth backing
   *  the wiki page; leaving an orphan raw produces "auto-recreate"
   *  cycles on the next lint pass). */
