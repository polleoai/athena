...
if [[ "$FULL_VAULT" == "1" ]]; then
  ...
  # vault needs Athena's Python backend (bin/) and config (config/).
  # Without these the chat will surface "Wiki synthesis was skipped —
  # Athena's Python backend isn't installed" after each kb add.
  echo "Copying Python backend (bin/ + config/) for full wiki synthesis..."
  if command -v rsync >/dev/null 2>&1; then
    rsync -a --delete "$REPO_ROOT/bin/"    "$VAULT/bin/"
    rsync -a --delete "$REPO_ROOT/config/" "$VAULT/config/"
  else
    rm -rf "$VAULT/bin" "$VAULT/config"
    cp -r "$REPO_ROOT/bin"    "$VAULT/bin"
    cp -r "$REPO_ROOT/config" "$VAULT/config"
  fi
  echo "Installed Python backend into $VAULT/{bin,config}/"
  echo "Note: Python 3.10+ must be available in the vault's environment for synthesis to run."
fi
