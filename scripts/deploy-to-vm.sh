...
#   ./scripts/deploy-to-vm.sh --full-vault # also ship Python backend for synthesis
#
...
# Stage at vault-root level so the same tarball can carry plugin dirs
# (.obsidian/plugins/...) AND optional Python backend (bin/, config/)
# without different tar roots per mode. The VM extracts from the vault
# root, not from inside .obsidian/.
STAGE=$(mktemp -d -t athena-deploy-XXXXXX)
trap 'rm -rf "$STAGE"' EXIT
mkdir -p "$STAGE/.obsidian/plugins/athena"
cp "$ATHENA_SRC/main.js"       "$STAGE/.obsidian/plugins/athena/"
cp "$ATHENA_SRC/manifest.json" "$STAGE/.obsidian/plugins/athena/"
cp "$ATHENA_SRC/styles.css"    "$STAGE/.obsidian/plugins/athena/"
# 1.0.9+ bundles Athena's Python synthesis sources inside the plugin
# install dir (.obsidian/plugins/athena/bin/). Earlier deploys only
# shipped main.js/manifest.json/styles.css — the plugin then looked
# for bin/lib/wiki_page.py inside its install dir and ENOENT'd because
# the deploy script left bin/ behind on the builder's machine.
if [[ -d "$ATHENA_SRC/bin" ]]; then
  cp -r "$ATHENA_SRC/bin" "$STAGE/.obsidian/plugins/athena/"
fi
if [[ "$ATHENA_ONLY" == "0" ]]; then
  mkdir -p "$STAGE/.obsidian/plugins/gryphon"
  cp "$GRYPHON_SRC/main.js"       "$STAGE/.obsidian/plugins/gryphon/"
  cp "$GRYPHON_SRC/manifest.json" "$STAGE/.obsidian/plugins/gryphon/"
  cp "$GRYPHON_SRC/styles.css"    "$STAGE/.obsidian/plugins/gryphon/"
  if [[ -d "$GRYPHON_SRC/hooks" ]]; then
    cp -r "$GRYPHON_SRC/hooks" "$STAGE/.obsidian/plugins/gryphon/"
  fi
fi
if [[ "$FULL_VAULT" == "1" ]]; then
  # Wiki synthesis runs `python3 bin/lib/wiki_page.py` inside the vault.
  # Without bin/, the test VM is in capture-only mode (raw saved, no
  # wiki page) — fine for testing the plugin runtime, but masks bugs
  # in the synthesis path. We also ship RULES.md if present so the
  # LLM prompt picks up the user's naming/tagging conventions (the
  # plugin has an inline fallback so RULES.md is optional).
  echo "==> including Python backend (bin/) for full wiki synthesis"
  cp -r bin "$STAGE/bin"
  if [[ -f RULES.md ]]; then
    cp RULES.md "$STAGE/RULES.md"
    echo "    + RULES.md (naming/tagging conventions)"
  fi
fi

# Build the tar arg list from what actually got staged. Earlier versions
# always listed "config" even though Athena doesn't have a config/ dir,
# causing tar to fail loudly when --full-vault was passed. Building from
# disk reality avoids that whole class.
TAR_ITEMS=(.obsidian)
if [[ "$FULL_VAULT" == "1" ]]; then
  [[ -d "$STAGE/bin" ]]     && TAR_ITEMS+=(bin)
  [[ -f "$STAGE/RULES.md" ]] && TAR_ITEMS+=(RULES.md)
fi

TARBALL=/tmp/athena-plugin.tar.gz
SUFFIX="Athena + Gryphon"
[[ "$ATHENA_ONLY" == "1" ]] && SUFFIX="Athena only"
[[ "$FULL_VAULT" == "1" ]] && SUFFIX="$SUFFIX + Python backend"
echo "==> packaging: $SUFFIX"
(cd "$STAGE" && tar --no-xattrs -czf "$TARBALL" "${TAR_ITEMS[@]}")
...
HOST_IP=\$(ip route | awk '/default/ { print \$3 }') && cd ~/athena-test-vault && curl -fSL http://\$HOST_IP:$PORT/athena-plugin.tar.gz -o /tmp/a.tar.gz && tar xzf /tmp/a.tar.gz --overwrite && rm /tmp/a.tar.gz && ls -l .obsidian/plugins/athena/main.js .obsidian/plugins/gryphon/main.js 2>/dev/null
...
\$HOST_IP = '$HOST_IP'; \$VAULT = "\$env:USERPROFILE\athena-test-vault"; New-Item -ItemType Directory -Force -Path "\$VAULT\.obsidian" | Out-Null; Set-Location \$VAULT; Invoke-WebRequest "http://\${HOST_IP}:$PORT/athena-plugin.tar.gz" -OutFile a.tar.gz; tar xzf a.tar.gz; Remove-Item a.tar.gz; Get-ChildItem .obsidian\plugins\athena\main.js,.obsidian\plugins\gryphon\main.js -ErrorAction SilentlyContinue | Format-List Name,Length,LastWriteTime
