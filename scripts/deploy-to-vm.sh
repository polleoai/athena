...
if [[ "$FULL_VAULT" == "1" ]]; then
  ...
  # Without these dirs the test VM is in capture-only mode (raw saved,
  # no wiki page) — fine for testing the plugin runtime, but masks any
  # bug in the synthesis path. Adds significant tarball size on first
  # deploy; subsequent --no-build re-deploys reuse the same tarball.
  echo "==> including Python backend (bin/ + config/) for full wiki synthesis"
  cp -r bin    "$STAGE/bin"
...
(cd "$STAGE" && tar --no-xattrs -czf "$TARBALL" .obsidian $([[ "$FULL_VAULT" == "1" ]] && echo "bin config"))
